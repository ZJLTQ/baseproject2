...
public class Digests {
	 ...
	 * 对输入字符串进行md5散列，带salt达到更高的安全性.
	 */
	public static byte[] md5(String input, Charset charset, byte[] salt) {
		return digest(input.getBytes(charset), MD5, salt, 1);
	}
	...
	/**
	 * 对输入字符串进行md5散列，带salt而且迭代达到更高更高的安全性.
	 */
	...
	public static byte[] md5(String input, byte[] salt, int iterations) {
		return digest(input.getBytes(Charsets.UTF8), MD5, salt, iterations);
	}
	...
	public static byte[] md5(String input, Charset charset, byte[] salt, int iterations) {
		return digest(input.getBytes(charset), MD5, salt, iterations);
	}
